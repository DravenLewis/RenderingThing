...
float sampleShadow2D(int shadowType, int mapIndex, vec4 lightSpacePos, float bias, float filterScale, vec3 receiverPos, float receiverGradScale) {
    ...
    if(shadowType != 0){
        float kernelRadius = (shadowType == 1) ? 1.15 : 1.85;
        float texelRadius = kernelRadius * clampedFilterScale * max(texelSize.x, texelSize.y);
        float rpScale = (shadowType == 1) ? 0.70 : 0.95;
        float rpCap = (shadowType == 1) ? 0.0012 : 0.0020;
    ...
    if(shadowType == 1){
        ...
        for(int i = 0; i < tapCount; ++i){
            vec2 tapOffset = (kernelRot * kernel[i]) * pcfTexel * 1.15;
        ...
        for(int i = 0; i < tapCount; ++i){
            vec2 tapOffset = (kernelRot * kernel[i]) * pcfTexel * 1.85;
            vec2 sampleUv = clamp(projCoords.xy + tapOffset, uvMin, uvMax);
...
float computeShadowBias(Light light, vec3 normal, vec3 fragPos){
    ...
    if(lightType == 1){
        float baseBias = baseOffset * 0.0045;
        float slopeBias = normalOffset * (0.0009 + slope * 0.0035);
        return clamp(baseBias + slopeBias, 0.00003, 0.00038);
    }else if(lightType == 2){
...
vec3 offsetShadowReceiver(Light light, vec3 normal, vec3 fragPos){
    ...
    float normalOffset = max(light.shadow.y, 0.0);
    if(lightType == 1){
        float worldOffset = normalOffset * 0.010 + baseOffset * 0.0035;
        worldOffset = clamp(worldOffset, 0.0, 0.00012);
